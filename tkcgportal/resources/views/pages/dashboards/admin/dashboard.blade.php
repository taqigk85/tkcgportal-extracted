...
<div class="flex gap-4 overflow-x-auto">
    <!-- Card 1: Projects -->
    <div class="w-[280px] lg:w-[240px] border border-[#ccc] p-5 rounded-xl shadow-md flex-shrink-0">
    ...
    <div class="w-[280px] lg:w-[240px] border border-[#ccc] p-5 rounded-xl shadow-md flex-shrink-0">
      <div class="w-[50px] h-[50px] rounded-full flex justify-center items-center border border-gray-300">
        <img src="{{ asset('public/images/agenda-bookmark@2x.png') }}" alt="Clients Icon" class="w-[24px] h-[24px]">
      </div>
      <p class="text-sm text-gray-600 mt-2">Clients</p>
      <h2 class="text-2xl font-bold text-black my-1.5">{{ $usersCount }}</h2>
    </div>
   </div>
  
  
  @if($Projects && $Projects->count() > 0)
  <div class="flex pt-7 gap-4 2xl:flex-col">
   <div class="w-1/2 2xl:w-full">
      <div class="">
          <h3 class="font-normal">Projects</h3>
      </div>
    <div class="flex flex-col justify-between mt-2 shadow-md rounded-md md:overflow-scroll table_height">
      ...
      <thead>
          <tr>
              ...
              <th class="bg-[#49b747] text-white border border-white p-[15px_5px] text-left align-baseline whitespace-normal">Client Name</th>
          ...
          <tr>
             <td class="!align-middle text-center rounded-bl-[6px] whitespace-normal p-[5px] text-[#333] border border-[#dddddd]">{{ $index + 1 }}</td>
              <td class="!align-middle whitespace-normal p-[5px] text-[#333] border border-[#dddddd] text-left">
               ...
               <td class="!align-middle whitespace-normal p-[5px] text-[#333] border border-[#dddddd] text-left">{{ucwords(Helper::getUsernameById($project->userId))}}</td>
               <td class="!align-middle whitespace-normal p-[5px] text-[#333] border border-[#dddddd] text-left">
  ...
  <div class="flex justify-end">
   ...
   <div class="w-1/2 2xl:w-full">
    <div class="">
        <h3 class="font-normal">Clients</h3>
    </div>
  <div class="flex flex-col justify-between mt-2 shadow-md rounded-md md:overflow-scroll table_height">
    <table class="mt-0 w-full border-collapse border-spacing-0 overflow-hidden">
    <thead>
    ...
    <tbody>
        ...
        <tr>
           <td class="!align-middle text-center rounded-bl-[6px] whitespace-normal p-[5px] text-[#333] border border-[#dddddd]">{{ $index + 1 }}</td>
            <td class="!align-middle whitespace-normal p-[5px] text-[#333] border border-[#dddddd] text-left">{{ ucfirst($user->name) }}</td>
            <td class="!align-middle whitespace-normal p-[5px] text-[#333] border border-[#dddddd] text-left rounded-br-[6px]">
                    <div class="list-action-container flex items-center justify-center m-0 -mx-[6px]">
...
<div class="flex justify-end">
  <a href="{{ route('admin.client.list') }}" class="inline-block mt-4 px-4 py-2 bg-gray-800 text-white rounded-md no-underline text-center uppercase text-sm">View All</a>
</div>
 </div>
 </div>
 @endif
